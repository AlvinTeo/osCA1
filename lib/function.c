...
int write() {
        ...
        fprintf(memoryOutput,"Address\t | Frame\t | Content\n");
        fprintf(memoryOutput,"-------\t | --------\t | --------\n");
        for(int i = 0; i < pageTime ; i++){
            fprintf(memoryOutput,"0x0%x\t | %d\t\t |\n",i ,(int)i/256);
        }
        for(int i = pageTime; i <= totalPagePrinted; i++){
            char c = (rand()%(126-33)) + 33;
            memory[i] = c;
            fprintf(memoryOutput,"0x%x\t | %d\t\t | %c\n",i ,(int)i/256 ,c);
        }
        for(int i = totalPagePrinted; i <= MEMORY; i++){
            fprintf(memoryOutput,"0x%x\t | %d\t\t |\n",i ,(int)i/256);
        }


        fprintf(pageOutput,"Page\t | Page Table Entry\n");
        fprintf(pageOutput,"-------\t | --------\n");
        for(int i = 0; i < pageTime ; i++){
            fprintf(memoryOutput,"0x0%x\t | %d\t\t |\n",i ,(int)i/256);
        }
